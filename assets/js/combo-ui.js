...
(function () {
  ...
  function initialize() {
    ...
    panel.innerHTML = '<section class="combo-workspace"><div class="combo-heading"><div><h3>🔗 스킬·콤보 순서 계산</h3><p>아이콘을 눌러 해당 순번의 스킬과 콤보 효과를 편집합니다.</p></div><strong id="comboMpReadout" class="combo-mp-readout"></strong></div><div class="combo-range-choice" aria-label="대미지 거리 판정"><span>거리 판정</span><label><input type="checkbox" id="comboRangeShort"> 근거리</label><label><input type="checkbox" id="comboRangeLong"> 원거리</label><small id="comboRangeStatus">둘 다 해제: 스킬 기본값</small></div><div id="comboChain" class="combo-chain" aria-label="콤보 스킬 순서"></div><div id="comboEntryEditor" class="combo-entry-editor"></div><p class="combo-order-note">최대 MP = 100 + 레벨 + INT × 0.1 + 장비·크리스타·스킬의 최대 MP. MP 부족 시 해당 스킬부터 계산하지 않습니다.</p><div id="comboSequenceStatus" role="status"></div><div id="comboSequenceOutput"></div></section>';
    ...
    if (window.ToramOptimizationPreferences && window.ToramOptimizationPreferences.bindRangeControls) window.ToramOptimizationPreferences.bindRangeControls();
