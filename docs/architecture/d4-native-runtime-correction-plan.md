...
N0~N6 완료 이력은 기존 기능 연결을 뜻하며 추가 성능 개선 완료를 뜻하지 않는다. 현 구조 감사의 후속 후보는 작업 묶음마다 스레드 생성 비용, 계산과 무관한 UI 입력의 session 폐기, 결과 오류 표현이다. 사용자 선택·재현·회귀를 갖춘 후 수정한다. 보류된 피격/콤보 엔진을 이 작업에 포함하지 않는다.
