...
assert.match(optimizerUi, /launchD4Worker\(problem, currentEvaluation, locks, version, 30000\)/, '결과 탭은 30초 전역 계산 예산을 시작해야 합니다.');
assert.match(optimizerUi, /ToramD4NativeClient\.resume\([\s\S]*timeLimitMs:30000/u, 'bounded native 결과는 같은 frontier를 30초 정밀 계산으로 재개해야 합니다.');
assert.match(optimizerUi, /continueButton\.hidden = !\(\(status === 'bounded' \|\| status === 'no-incumbent-yet' \|\| status === 'paused'\) && state\.continuationId\)/, '세션이 없는 종료 결과에는 정밀 계산 재개 버튼을 표시하면 안 됩니다.');
assert.match(optimizerUi, /ToramD4NativeClient\.pause/, '네이티브 계산은 취소와 구분되는 일시정지를 제공해야 합니다.');
assert.match(optimizerUi, /renderD4RecommendationOverview/, '전역 추천 결과는 전용 현재↔추천 비교 렌더러를 거쳐야 합니다.');
assert.match(optimizerUi, /최대 HP[\s\S]*최대 MP[\s\S]*공격 MP 회복[\s\S]*ASPD/, '추천 비교에는 HP·MP·공격 MP 회복·속도 유틸리티가 포함돼야 합니다.');
assert.match(optimizerUi, /계산 상세 및 최적화 검증/, '엔진·평가·상한 정보는 일반 추천 화면이 아닌 계산 상세에 둬야 합니다.');
